...
def find_low_rsi_stocks():
    ...
    for index, ticker in enumerate(tickers_df['Ticker'], 1):
        try:
            ...
            df['RSI'] = menu.calculate_rsi(df['종가'])
    ...
    if low_rsi_stocks:
        ...
        print("\nStocks with RSI below 30 and market cap above 500 billion KRW:")
        print(result_df)
        result_df.to_csv('low_rsi_high_cap_stocks.csv', index=False)
        print("Results exported to 'low_rsi_high_cap_stocks.csv'")
    else:
        print("\nNo stocks found with RSI below 30 and market cap above 500 billion KRW.")
